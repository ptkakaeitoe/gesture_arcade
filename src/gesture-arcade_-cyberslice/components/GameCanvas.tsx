...
import type { Entity, Point, DifficultyConfig, NormalizedPoint } from "../types";
...
interface GameCanvasProps {
  ...
  handPosition?: NormalizedPoint | null;
  isActive?: boolean;
}
...
const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  const spawnRateBase = difficulty.spawnRateBase;
  const bombChance = difficulty.bombChance;
  ...
  const loop = useCallback(() => {
    ...
    requestRef.current = requestAnimationFrame(loop);
  }, [difficulty, onGameOver, updateScore]);
  ...
  useEffect(() => {
    ...
    const canvasX = handPosition.x * canvas.width;
    const canvasY = handPosition.y * canvas.height;

    if (!isMouseDown.current) {
      handleInputStart(canvasX, canvasY);
    } else {
      handleInputMove(canvasX, canvasY);
    }
  }, [handPosition]);
